...
NostrString NWC::sendEvent(SignedNostrEvent *event) {
    NostrString subId = this->pool->subscribeMany(
        ...
        [&](const String &subId, const String &reason) {
            Utils::log("NWC: closed subscription: " + reason);
        },
...
void NWC::payInvoice(
    NostrString invoice, unsigned long amount,
    std::function<void(PayInvoiceResponse)> onRes,
    ...
    std::unique_ptr<NWCResponseCallback<PayInvoiceResponse>> callback(
        new NWCResponseCallback<PayInvoiceResponse>());
...
void NWC::multiPayInvoice(std::initializer_list<Invoice> invoices,
                          std::function<void(MultiPayInvoiceResponse)> onRes,
                          std::function<void(NostrString,NostrString)> onErr) {
    SignedNostrEvent ev = this->nip47.multiPayInvoice(invoices);
    std::unique_ptr<NWCResponseCallback<MultiPayInvoiceResponse>> callback(
        new NWCResponseCallback<MultiPayInvoiceResponse>());
...
void NWC::payKeySend(NostrString pubkey, unsigned long amount,
                     NostrString preimage,
                     std::initializer_list<TLVRecords> tlv,
                     std::function<void(PayKeySendResponse)> onRes,
                     std::function<void(NostrString,NostrString)> onErr) {
    SignedNostrEvent ev = this->nip47.payKeySend(pubkey, amount, preimage, tlv);
    // NWCResponseCallback<PayKeySendResponse> callback;
    std::unique_ptr<NWCResponseCallback<PayKeySendResponse>> callback(
        new NWCResponseCallback<PayKeySendResponse>());
    callback->onRes = onRes;
    callback->onErr = onErr;
    callback->timestampSeconds = Utils::unixTimeSeconds();
    callback->eventId = ev.getId();
...
void NWC::multiPayKeySend(std::initializer_list<KeySend> keySends,
                          std::function<void(MultiPayKeySendResponse)> onRes,
                          std::function<void(NostrString,NostrString)> onErr) {
    SignedNostrEvent ev = this->nip47.multiPayKeySend(keySends);
    // NWCResponseCallback<MultiPayKeySendResponse> callback;
    std::unique_ptr<NWCResponseCallback<MultiPayKeySendResponse>> callback(
        new NWCResponseCallback<MultiPayKeySendResponse>());
    callback->onRes = onRes;
    callback->onErr = onErr;
    callback->timestampSeconds = Utils::unixTimeSeconds();
    callback->eventId = ev.getId();
...
void NWC::makeInvoice(unsigned long amount, NostrString description,
                      NostrString descriptionHash, unsigned long expiry,
                      std::function<void(MakeInvoiceResponse)> onRes,
                      std::function<void(NostrString,NostrString)> onErr) {
    SignedNostrEvent ev =
        this->nip47.makeInvoice(amount, description, descriptionHash, expiry);
    std::unique_ptr<NWCResponseCallback<MakeInvoiceResponse>> callback(
        new NWCResponseCallback<MakeInvoiceResponse>());
    callback->onRes = onRes;
    callback->onErr = onErr;
    callback->timestampSeconds = Utils::unixTimeSeconds();
    callback->eventId = ev.getId();
...
void NWC::lookUpPaymentHash(NostrString paymentHash,
                            std::function<void(LookUpInvoiceResponse)> onRes,
                            std::function<void(NostrString,NostrString)> onErr) {
    SignedNostrEvent ev = this->nip47.lookUpPaymentHash(paymentHash);
    // NWCResponseCallback<LookUpInvoiceResponse> callback;
    std::unique_ptr<NWCResponseCallback<LookUpInvoiceResponse>> callback(
        new NWCResponseCallback<LookUpInvoiceResponse>());
    callback->onRes = onRes;
    callback->onErr = onErr;
    callback->timestampSeconds = Utils::unixTimeSeconds();
    callback->eventId = ev.getId();
...
void NWC::listTransactions(unsigned long from, unsigned long until, int limit,
                           int offset, bool unpaid, NostrString type,
                           std::function<void(ListTransactionsResponse)> onRes,
                           std::function<void(NostrString,NostrString)> onErr) {
    SignedNostrEvent ev = this->nip47.listTransactions(from, until);
    // NWCResponseCallback<ListTransactionsResponse> callback;
    std::unique_ptr<NWCResponseCallback<ListTransactionsResponse>> callback(
        new NWCResponseCallback<ListTransactionsResponse>());
    callback->onRes = onRes;
    callback->onErr = onErr;
    callback->timestampSeconds = Utils::unixTimeSeconds();
    callback->eventId = ev.getId();
...
void NWC::getBalance(std::function<void(GetBalanceResponse)> onRes,
                     std::function<void(NostrString,NostrString)> onErr) {
    SignedNostrEvent ev = this->nip47.getBalance();
    // NWCResponseCallback<GetBalanceResponse> callback;
    std::unique_ptr<NWCResponseCallback<GetBalanceResponse>> callback(
        new NWCResponseCallback<GetBalanceResponse>());
    callback->onRes = onRes;
    callback->onErr = onErr;
    callback->timestampSeconds = Utils::unixTimeSeconds();
    callback->eventId = ev.getId();
...
void NWC::getInfo(std::function<void(GetInfoResponse)> onRes,
                  std::function<void(NostrString,NostrString)> onErr) {
    SignedNostrEvent ev = this->nip47.getInfo();
    // NWCResponseCallback<GetInfoResponse> callback;
    std::unique_ptr<NWCResponseCallback<GetInfoResponse>> callback(
        new NWCResponseCallback<GetInfoResponse>());
    callback->onRes = onRes;
    callback->onErr = onErr;
    callback->timestampSeconds = Utils::unixTimeSeconds();
    callback->eventId = ev.getId();
