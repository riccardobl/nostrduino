...
namespace nostr {
    ...
    class Nip47Response {
        public:
         ...
         Nip47Response(NostrString errorCode, NostrString errorMessage,
                       NostrString resultType, T result)
             : errorCode(errorCode),
   ...
   typedef struct s_LookUpInvoiceResponse {
    ...
    class Nip47 {
       public:
        Nip47(){};
         Nip47(Nip04 nip04, NostrString servicePubKey,
                    NostrString userPrivKey)
            : servicePubKey(servicePubKey),
              userPrivKey(userPrivKey),
              nip04(nip04) {}
        SignedNostrEvent payInvoice(NostrString invoice,
                                      unsigned long amount = -1);
        SignedNostrEvent multiPayInvoice(
            std::initializer_list<Invoice> invoices);
        SignedNostrEvent payKeySend(NostrString pubkey, unsigned long amount,
                                    NostrString preimage = "",
                                    std::initializer_list<TLVRecords> tlv = {});
        SignedNostrEvent multiPayKeySend(
            std::initializer_list<KeySend> keySends);
        SignedNostrEvent makeInvoice(unsigned long amount,
                                     NostrString description = "",
                                     NostrString descriptionHash = "",
        ...
        SignedNostrEvent listTransactions(unsigned long from = 0,
        ...
        void parseResponse(SignedNostrEvent* response,
       ...
       NostrString userPrivKey;
       Nip04 nip04;
       SignedNostrEvent createEvent(NostrString method,
                                    JsonDocument doc);
       };
}  // namespace nostr
