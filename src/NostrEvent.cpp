...
bool SentNostrEventStatus::waitAll(bool fireException , EventSubmissionFailureCallback callback )
{
    bool failed = false;
    for (auto &x : this->statusXrelay)
    {
        if (!x.second.accepted)
        {
            failed = true;
            if (fireException && NostrString_length(x.second.error) > 0)
            {
                // Serial.println("Error: " + x.second.error);
                Utils::log("Error: " + x.second.error);
                throw std::runtime_error(
                    NostrString_toChars(x.second.error));
            }
            else
            {
                if (callback != nullptr)
                {
                    callback(x.second.message);
                }
            }
        }
    }
...
bool SentNostrEventStatus::waitSome(unsigned int n, EventSubmissionFailureCallback callback )
{
